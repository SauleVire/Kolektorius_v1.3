...
MenuBackend menu = MenuBackend(menuUseEvent,menuChangeEvent); // menu design
   //                        ("                ")
   MenuItem P1 =  MenuItem("  COLLECTOR   ",1);
      MenuItem P11 = MenuItem("difference on ",2);
      MenuItem P12 = MenuItem("difference off",2);
      MenuItem P13 = MenuItem("pump on       ",2);


   MenuItem P2 = MenuItem(" THERMOSTAT    ",1);//"TERMOSTATAS   "
      MenuItem P21 = MenuItem("themperature 1",2);//"themperature 1"
      MenuItem P22 = MenuItem("status        ",2);//"status        "


   MenuItem P3 = MenuItem("  SETTINGS   ",1);
      MenuItem P31 = MenuItem("save          ",2);
      MenuItem P32 = MenuItem("default       ",2);
      MenuItem P33 = MenuItem("brightness    ",2);
...
/* __________________________Settings brithness __________________ */
  ...
  {             
  myGLCD.print("brightness", 0, 40); // keiciamos reikšmes pavadinimas
...
if (lcd_backlight < 1)  lcd_backlight = 10;
            ...
            myGLCD.print("0% ",60,40);
             ...
             if(action==4) // jesli wcisnieto OK 
               {
                 myGLCD.print(">brightness OK",0,32);delay(2000); // pokazujemy OK przez 2 sek.
...
// Save to EEPROM
     if (used.item.getName() == "save      ")   // exactly the same string "Save          "
      {
                 SaveConfig();
                 myGLCD.print(">saved OK     ", 0, 32);delay(2000); // show OK for 2 sec
...
// Save to EEPROM
     if (used.item.getName() == "default       ")   // exactly the same string "Save          "
      {
                 myGLCD.print(">default OK   ", 0, 32);delay(2000); // show OK for 2 sec
                 myGLCD.print("              ", 0, 40); // clear line
                 myGLCD.print(Nokia_LCD_string_1, CENTER, 24);           // reconstruct the previous state at LCD
...
Pump_power_on_difference =  MeniuFunkcija (" Set =        ", Pump_power_on_difference, 25, 1, ">Difference OK");
     ///////////////////////////////////////////////////////////////////
/* __________________________ Collector _______________________ */
// OFF - the difference between the temperature           
if (used.item.getName() == "difference off")   // exactly the same string "Difference off"
Pump_power_off_difference =  MeniuFunkcija (" Set =        ", Pump_power_off_difference, 25, 1, ">Difference OK");
     ///////////////////////////////////////////////////////////////////    

/* __________________________ Collector Manual pump on ____________________________________ */
 if (used.item.getName() == "pump on       ")
 {      
        myGLCD.print("pump    ", 0, 32);
...
if(action==0) {Manual_pump_status = true;  myGLCD.print("on ", RIGHT, 32);delay(200);}
             if(action==4) // 0
               {
                 myGLCD.print(">pump OK       ", RIGHT, 32); delay(2000); // 0
...
if (used.item.getName() == "themperature 1")   // exactly the same string "temperature 1 "
temperature_1 =  MeniuFunkcija ("temp 1=    ", temperature_1, 99, -25, "Temperature OK");
     ///////////////////////////////////////////////////////////////////
...
if (used.item.getName() == "status        ")
 {      
        //tft.write(7);    
        myGLCD.print("status-", LEFT, 32);
        if (Thermostat_status == 1) myGLCD.print("heating ", RIGHT, 40); // heating
        if (Thermostat_status == 2) myGLCD.print("freezing", RIGHT, 40); // freezing
        if (Thermostat_status == 3) myGLCD.print("     off", RIGHT, 40); // turned off
        ...
        while(action!=4)                   //
         {
           ...
           if(Keyboard_change!=action)          
             ...
             if(action==4) // 0
               {
                  myGLCD.print(">status     OK", LEFT, 32); delay(2000); // 0
...
myGLCD.print("Collector ", 0, 0);
myGLCD.print("Boiler    ", 0, 8);
myGLCD.print("ThermStat ", 0, 16);
myGLCD.invert(true);
...
myGLCD.print("T- ", 0, 40); 
    if (Thermostat_status == 1) myGLCD.print("heating ", 18, 40); // heating
    if (Thermostat_status == 2) myGLCD.print("freezing", 18, 40); // freezing
    if (Thermostat_status == 3) myGLCD.print("     off", 18, 40); // turned off
}
